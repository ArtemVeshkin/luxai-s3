...
class Rulebased:
    ...
    def get_barrier_coords(self, state: State):
        ...
        for relic_node in state.space.relic_nodes:
            ...
            barrier_coord_shift = (+3, +3) if state.team_id == 0 else (-3, -3)
            relic_main_barrier_coord = (
                relic_node.x + barrier_coord_shift[0],
                relic_node.y + barrier_coord_shift[1]
            )
            for coord_shift in range(4):
                if state.team_id == 0:
                    coord_shift *= -1
                
                barrier_coords.append((
                    relic_main_barrier_coord[0] + coord_shift,
                    relic_main_barrier_coord[1]
                ))
                if coord_shift != 0:
                    barrier_coords.append((
                        relic_main_barrier_coord[0],
                        relic_main_barrier_coord[1] + coord_shift
                    ))
        ...
        barrier_coords = list(filter(coord_filter, barrier_coords))
        n_barrier_coords = len(barrier_coords)
        if n_barrier_coords < 8:
            ...
            center_barrier_coords = list(filter(coord_filter, center_barrier_coords))
            for i in range(min(8 - n_barrier_coords, len(center_barrier_coords))):
                barrier_coords.append(center_barrier_coords[i])
        return barrier_coords
    ...
    def build_barrier(self, state: State):
        ...
        for ship in state.fleet:
            ...
            if actions or ship.coordinates == target:
                barrier_to_n_ships[target] += 1
                if barrier_to_n_ships[target] >= 2:
