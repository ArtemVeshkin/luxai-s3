...
class Ship:
    ...
    def can_sap(self, node):
        if self.node:
            ...
            if abs(node.coordinates[0] - coords[0]) <= Global.UNIT_SAP_RANGE and abs(node.coordinates[1] - coords[1]) <= Global.UNIT_SAP_RANGE:
                return True
            else:
                return False
        else:
    ...
    def project_energy(self, ship, step):
        ...
        if nebula_diff and not Global.ENERGY_NODES_MOVEMENT_STATUS[-1] and not Global.OBSTACLES_MOVEMENT_STATUS[-1]:
            if not ship.expected_is_enemy and len(ship.track) > 0 and len(ship.energies) > 0 and ship.energy !=0:
                move_cost = (0 if ship.track[-1][0] == ship_coords[0] and ship.track[-1][1] == ship_coords[1] else Global.UNIT_MOVE_COST)
            ...
            if ship.energy >= Global.UNIT_SAP_COST:
                if (ship.task == 'harvest' and ship.action == ActionType.center) or (ship.task != 'harvest'):
                    ...
                    if max_score > 0:
                        ...
                        ship.action_sap_info = (max_node.coordinates[0] - ship.coordinates[0], max_node.coordinates[1] - ship.coordinates[1])

    def get_sap_score(self, node):
            
        score = 0
        allies_coords = [ship.coordinates for ship in self.fleet]
        def is_ally(x, y):
            for a_c in allies_coords:
                if a_c[0] == x and a_c[1] == y:
                    return True
            return False

        if not self.space.get_node(*node.coordinates).is_enemy:
            #we shoot, but do not harm
            score -= Global.UNIT_SAP_COST
            if is_ally(node.coordinates[0], node.coordinates[1]):
                score -= Global.UNIT_SAP_COST
                
        col_dmg = Global.UNIT_SAP_COST * Global.UNIT_SAP_DROPOFF_FACTOR
        ...
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
                if i == 0 and j == 0:
                    ...
                    if node.coordinates[0] + i >= Global.SPACE_SIZE:
                        pass
                    elif node.coordinates[0] + j >= Global.SPACE_SIZE:
                        pass 
                    elif node.coordinates[0] + i <= 0:
                        pass
                    elif node.coordinates[0] + j <= 0:
                        pass 
                    else:
                        is_enemy = int(self.space.get_node(node.coordinates[0] + i,
                                                           node.coordinates[0] + j).is_enemy) 
                        score += is_enemy * col_dmg - int(is_ally(node.coordinates[0] + i,
                                                                  node.coordinates[0] + j)) * col_dmg
        return score
