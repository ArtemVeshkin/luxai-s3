...
def create_weights(space, coords=None, max_dist=None, energy_update_in=None, warp_distance=None):
    """
    Create a weights grid for the A* algorithm based on node properties.

    Parameters:
        space (iterable): Collection of node objects.
        coords (tuple, optional): Reference coordinates.
        max_dist (int, optional): Nodes further than this Manhattan distance get a weight of -1.
        energy_update_in (int, optional): Beyond this distance the node's energy is reset.
        warp_distance (int, optional): For nodes exactly this Manhattan distance away, use warp_point
                                       to update is_walkable and type.
    ...
    for node in space:
        # Calculate Manhattan distance if we have a reference coordinate.
        distance = manhattan_distance(coords, node.coordinates) if coords else None

        # Nodes beyond the maximum allowed distance get an impassable weight.
        if coords and max_dist is not None and distance > max_dist:
            weight = -1
        else:
            if coords and warp_distance is not None and distance > warp_distance and Global.OBSTACLE_MOVEMENT_DIRECTION_FOUND:
                direction = Global.OBSTACLE_MOVEMENT_DIRECTION
                warped_node = space.get_node(*warp_point(node.x + direction[0], node.y + direction[1]))
                node_walkable = warped_node.is_walkable
                node_type = warped_node.type
            else:
                node_walkable = node.is_walkable
                node_type = node.type

            # If the node (or warped node) is not walkable, set weight to -1.
            if not node_walkable:
                weight = -1
            else:
                # Use node energy, with a fallback if energy is not set.
                node_energy = node.energy if node.energy is not None else Global.HIDDEN_NODE_ENERGY

                # Reset node energy if the node is beyond the energy update threshold.
                if coords and energy_update_in is not None and distance > energy_update_in:
                    node_energy = Global.HIDDEN_NODE_ENERGY

                # Compute weight so that higher energy gives lower cost.
                weight = Global.MAX_ENERGY_PER_TILE + 1 - node_energy

                # If the node (or warped node) is of type nebula, adjust the weight.
                if node_type == NodeType.nebula:
                    weight += compute_nebula()
...
def compute_nebula():
    return Global.NEBULA_ENERGY_REDUCTION if len(Global.NEBULA_ENERGY_REDUCTION_SAMPLES) == 0 else abs(Global.NEBULA_ENERGY_REDUCTION_SAMPLES.most_common(1)[0][0])
