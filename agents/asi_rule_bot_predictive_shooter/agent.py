...
class Agent:
    ...
    def get_sap_score(self, target_node, forward_shooting=False):
        ...
        if target_enemy_count > 0:
            # Each enemy hit on target gives full benefit.
            score += target_enemy_count * base_cost
        else:
            # Missing enemy on the target penalizes the action.
            score -= base_cost

        # Penalize for any allied ships hit on the target.
        score -= target_ally_count * base_cost
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                if 0 <= adj_x < Global.SPACE_SIZE and 0 <= adj_y < Global.SPACE_SIZE:
                    ...
                    score += enemy_count * splash_cost - ally_count * splash_cost
