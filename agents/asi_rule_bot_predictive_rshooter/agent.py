...
class Node:
    ...
    def is_walkable(self) -> bool:
        return self.type != NodeType.asteroid  # & (not self.is_enemy)
...
class Space:
    ...
    def _update_map(self, obs, team_id):
    ...
    @staticmethod
    def steps_until_move(step):
        speed = 1.0 / Global.OBSTACLE_MOVEMENT_PERIOD
        current_step = get_match_step(step)
        t = (current_step - 1) / Global.OBSTACLE_MOVEMENT_PERIOD
        f = t - math.floor(t)

        # If the condition is met now, move happens immediately.
        if f < speed:
            return 0
        else:
            # Calculate the next step that meets (step - 1) / period is an integer.
            next_move_step = (math.floor(t) + 1) * Global.OBSTACLE_MOVEMENT_PERIOD + 1
            # Since steps are integer-valued, round up to the next integer number of steps.
            return int(math.ceil(next_move_step - current_step))
...
class Ship:
    ...
    def can_sap(self, node):
        if self.node:
            coords = self.coordinates
            if abs(node.coordinates[0] - coords[0]) <= Global.UNIT_SAP_RANGE and abs(
                    node.coordinates[1] - coords[1]) <= Global.UNIT_SAP_RANGE:
                return True
            else:
                return False
        else:
...
class Agent:
    ...
    def __init__(self, player: str, env_cfg) -> None:
        ...
        self.damage_map = np.zeros(shape=(Global.SPACE_SIZE,Global.SPACE_SIZE))
        self.shots = []
    ...
    def act(self, step: int, obs, remainingOverageTime: int = 60):
        ...
        self.find_relics(match_step)
        self.find_rewards(match_step)
        self.harvest(match_step)
        self.estimate_damage_drop_off(step)
        self.destroy()
        self.estimate_nebula_energy(step)
        return self.create_actions_array()

    def estimate_damage_drop_off(self, step):
        enemy_ships = self.opp_fleet.ships
        ships = self.fleet.ships
        for ship in ships + enemy_ships:
            for shot in self.shots:
                if ship.active and manhattan_distance(shot, ship.coordinates) == 1 and len(ship.energies) > 1:
                    cur_tile = self.space.get_node(*ship.coordinates)
                    cur_tile_energy = cur_tile.energy
                    move_cost = Global.UNIT_MOVE_COST
                    dmg = (ship.energies[-2] - ship.energy + cur_tile_energy - move_cost)

                    self.debug.append(
                        f"""Prev energy: {ship.energies[-2]} Cur energy: {ship.energy} Tile energy: {cur_tile_energy} Move cost energy: {move_cost} DMG: {dmg} out of {Global.UNIT_SAP_COST}\n"""
                    )
                    dmg /= Global.UNIT_SAP_COST
                    if cur_tile.type == NodeType.nebula:
                        dmg = None
                    else:
                        # if dmg in Global.POSSIBLE_SAP_DROPOFF_FACTORS:
                        Global.UNIT_SAP_DROPOFF_FACTOR.update([dmg])

                    if sum(Global.UNIT_SAP_DROPOFF_FACTOR.values()) == 5:
                        raise ValueError(self.debug)
                        print(Global.UNIT_SAP_DROPOFF_FACTOR)

        self.damage_map = np.zeros(shape=(Global.SPACE_SIZE, Global.SPACE_SIZE))
        self.shots = []

    def estimate_nebula_energy(self, step):
        ships = self.fleet.ships
        for ship in ships:
            if ship.active:
                self.project_energy(ship, step)
    ...
    def project_energy(self, ship, step):
        # get needed info
        ship_coords = ship.coordinates
        direction = ship.action.to_direction() if ship.action else (0, 0)
        cur_tile = self.space.get_node(ship_coords[0], ship_coords[1])
        target_tile = self.space.get_node(ship_coords[0] + direction[0],
        ...
        if nebula_diff and not Global.ENERGY_NODES_MOVEMENT_STATUS[-1] and not Global.OBSTACLES_MOVEMENT_STATUS[-1]:
            if not ship.expected_is_enemy and len(ship.track) > 0 and len(ship.energies) > 0 and ship.energy != 0:
                move_cost = (0 if ship.track[-1][0] == ship_coords[0] and ship.track[-1][1] == ship_coords[
                    1] else Global.UNIT_MOVE_COST)
                est = ship.energy - ship.energies[-1] - cur_tile_energy + move_cost
                if sum(Global.NEBULA_ENERGY_REDUCTION_SAMPLES.values()) < 10:
                    Global.NEBULA_ENERGY_REDUCTION_SAMPLES.update([est])
    ...
    def find_relics(self, match_step):
        ...
        def set_task(ship):
            ...
            path = astar(create_weights(self.space, ship.coordinates,
                                        max_dist = Global.MAX_STEPS_IN_MATCH + 1 - match_step,
                                        energy_update_in = match_step % Global.ENERGY_NODE_MOVEMENT_PERIOD + 1,
                                        warp_distance=None
                                        ),
    ...
    def find_rewards(self, match_step):
        ...
        for ship in self.fleet:
            if ship.energy >= Global.UNIT_SAP_COST:
                if (ship.task == 'harvest' and ship.action == ActionType.center) or (ship.task != 'harvest'):
                    valid_nodes = filter(ship.can_sap, self.space)
                    max_node = max(valid_nodes, key=self.get_sap_score)
                    max_score = self.get_sap_score(max_node)
                    if max_score > 0:
                        ...
                        self.damage_map[max_node.coordinates[0],  max_node.coordinates[1]] = Global.UNIT_SAP_COST
                        self.shots.append(max_node.coordinates)
    ...
    def get_sap_score(self, target_node, forward_shooting=True):
        ...
        """
        base_cost = Global.UNIT_SAP_COST
        splash_cost = Global.UNIT_SAP_COST * Global.UNIT_SAP_DROPOFF_FACTOR.most_common(1)[0][0]
        score = 0
        ...
        if forward_shooting:
            enemy_positions = [predict_ship_position(ship) for ship in self.opp_fleet]
        else:
            enemy_positions = [ship.coordinates for ship in self.opp_fleet]
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                if 0 <= adj_x < Global.SPACE_SIZE and 0 <= adj_y < Global.SPACE_SIZE:
                    # Get the node at the splash tile.
                    splash_node = self.space.get_node(adj_x, adj_y)
